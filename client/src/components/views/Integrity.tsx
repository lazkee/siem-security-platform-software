...
export default function Integrity({ integrityApi, queryApi }: IntegrityProps) {
    ...
    return (
        <div className="p-6 flex flex-col gap-6">
            <div className="flex flex-col justify-start items-stretch rounded-lg border-2 border-[#282A28] bg-[#1f2123] p-6 gap-6">
            ...
            ${status?.isChainValid !== false
                ...
                <div className="flex flex-col rounded-lg border-2 border-[#282A28] bg-[#1f2123] overflow-hidden">
                    <div className="flex justify-between items-center bg-[#1f1f1f] p-4 border-b border-[#333]">
                    ...
                    <div className="max-h-[600px] overflow-y-auto custom-scrollbar">
                        <table className="w-full table-fixed border-collapse text-sm">
                            <thead className="bg-[#2a2a2a] sticky top-0 z-10">
                                <tr>
                                    <th className="w-1/5 px-4 py-2 text-white text-center border-b border-[#2d2d2d]">ID</th>
                                    <th className="w-1/5 px-4 py-2 text-white text-center border-b border-[#2d2d2d]">Timestamp</th>
                                    <th className="w-1/5 px-4 py-2 text-white text-center border-b border-[#2d2d2d]">Type</th>
                                    <th className="w-1/5 px-4 py-2 text-white text-center border-b border-[#2d2d2d]">Integrity</th>
                                    <th className="w-1/5 px-4 py-2 text-white text-center border-b border-[#2d2d2d]">Digital Signature</th>
                                </tr>
                            </thead>
                            ...
                            <tbody>
                                {displayedEvents.length === 0 && !isLoading ? (
                                    <tr>
                                        <td colSpan={5} className="px-10 py-10 text-center text-[#a6a6a6] border-b border-[#2d2d2d]">
                                    ...
                                    displayedEvents.map((ev, i) => {
                                        ...
                                        return (
                                            <tr key={ev.id || i} className="transition-colors duration-200 hover:bg-[#2a2a2a]">
                                                <td className={`px-4 py-2 text-center font-semibold truncate ${compromised ? "text-red-400" : "text-white"}`}>
                                                    #{ev.id} {compromised && "⚠️"}
                                                </td>
                                                <td className="px-4 py-2 text-center text-white truncate">
                                                    {ev.timestamp ? new Date(ev.timestamp).toLocaleString("sr-RS") : "---"}
                                                </td>
                                                <td className={`px-4 py-2 text-center truncate ${compromised ? "text-red-400" : "text-gray-400"}`}>
                                                    {ev.eventType || "LOG"}
                                                </td>
                                                ...
                                                <td className="px-4 py-2 text-center font-mono truncate text-gray-400 max-w-[200px]">
